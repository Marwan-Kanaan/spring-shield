 ...
 * <h2>Backing off</h2>
 *
 * <p>The rule is that an explicit application bean always wins. Every default SpringShield
 ...
 *
 * <p>This composes with Spring Boot's own behaviour rather than fighting it. Boot's
 * default chain is guarded by {@code @ConditionalOnDefaultWebSecurity}, which in Spring
 * Boot 4.1 requires no {@code SecurityFilterChain} bean to be present. So when SpringShield
 * contributes a chain, Boot's default chain withdraws on its own, and the three cases stack
 * predictably:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Ordering matters for that to hold: SpringShield's auto-configuration must be applied
